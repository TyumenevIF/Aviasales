...
// При инициализации создается представление для отображения изображения, и два текстовых поля для отображения заголовка и текста.
- (instancetype)init {    
    self = [super init];
    if (self) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2 - 100.0,
                                                                   [UIScreen mainScreen].bounds.size.height/2 - 150.0, 200.0, 200.0)];
...
// для текстовых полей с помощью функции heightForText осуществляется расчет высоты
